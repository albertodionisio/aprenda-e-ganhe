...
const MIN_USD = parseFloat(process.env.WITHDRAWAL_MIN_USD || '10');
    ...
    async requestWithdrawal(req, res) {
        ...
        try {
            ...
            if (!amount || !currency || !paymentMethod) {
                return res.status(400).json({ error: "Informe valor, moeda e método de pagamento." });
            }

            const min = currency.toUpperCase() === 'MTN' ? MIN_MTN : MIN_USD;
            if (parseFloat(amount) < min) {
                return res.status(400).json({ error: `O valor mínimo de saque é ${min} ${currency.toUpperCase()}.` });
            }
            ...
            const { rows } = await client.query(
                ...
                [userId, amount, currency.toUpperCase(), paymentMethod, paymentDetails || null]
            );

            await client.query(
                `INSERT INTO transactions (user_id, type, amount, currency, status, payment_method)
                 VALUES ($1,'WITHDRAWAL',$2,$3,'PENDING',$4);`,
                [userId, amount, currency.toUpperCase(), paymentMethod]
            );

            await Notification.create(userId, {
                category: 'financial',
                title: 'Pedido de saque recebido',
                message: `O seu pedido de saque de ${amount} ${currency.toUpperCase()} está a ser processado.`
            }, client);
